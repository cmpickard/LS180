...
-- Write a SQL query that returns the same results as the answer for #3, but without
--  using DISTINCT.
...
--  represented in the films table, rounded to the nearest minute and listed in 
--  chronological order.

SELECT round(avg(duration)) AS avg_duration, trunc(year, -1) AS decade FROM films
  GROUP BY decade;
...
SELECT (trunc(year, -1)) AS decade, genre, string_agg(title, ', ') AS films FROM films
