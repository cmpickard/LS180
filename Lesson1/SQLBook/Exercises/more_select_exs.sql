-- Make sure you are connected to the encyclopedia database. Write a query to 
-- retrieve the first row of data from the countries table.
SELECT * 
FROM countries LIMIT 1;
...
-- Write a query to return each unique conservation status code alongside the number
--  of animals that have that code.
SELECT conservation_status, count(name)
FROM animals
GROUP BY conservation_status;


-- Connect to the ls_burger database. Write a query that returns the average burger
...
-- Write a query that returns the number of orders that include Fries and the number 
-- of orders that include Onion Rings.
SELECT side, count(side)
FROM orders
GROUP BY side HAVING (side = 'Fries' OR side = 'Onion Rings');
