...
SELECT first_name, last_name
FROM celebrities
WHERE occupation ILIKE '%sing%' AND occupation ILIKE '%act%';

-- Connect to the ls_burger database. Write a query that lists all of the burgers 
-- that have been ordered, from cheapest to most expensive, where the cost of the burger is less than $5.00.
SELECT burger
FROM orders
...
-- from any order worth 20 or more loyalty points. List the results from the highest 
-- number of points to the lowest.
