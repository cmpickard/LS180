...
-- Create a new column in the celebrities table called last_name. It should be 
-- able to hold strings of lengths up to 100 characters. This column should always hold a value.
ALTER TABLE celebrities NEW COLUMN last_name varchar(100) NOT NULL;

-- Change the celebrities table so that the date_of_birth column uses a data type 
-- that holds an actual date value rather than a string. Also ensure that this 
-- column must hold a value.
ALTER TABLE celebrities ALTER COLUMN date_of_birth TYPE date USING date_of_birth::date;

-- Change the max_weight_kg column in the animals table so that it can hold values 
-- in the range 0.0001kg to 200,000kg
...
-- Connect to the ls_burger database. Add the following columns to the orders table:
  ...
  -- A column called customer_loyalty_points that should hold integer values. If no
  --  value is specified for this column, then a value of 0 should be applied.
...
-- drink_cost to hold monetary values in dollars and cents (assume that all values 
-- will be less than $100). If no value is entered for these columns, a value of 0
--  dollars should be used.
ALTER TABLE orders ADD COLUMN burger_cost decimal(4, 2) DEFAULT 0;
ALTER TABLE orders ADD COLUMN side_cost decimal(4, 2) DEFAULT 0;
ALTER TABLE orders ADD COLUMN drink_cost decimal(4, 2) DEFAULT 0;
